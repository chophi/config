...
function __prompt_command_powerline {
    ...
    PS1+=""\\[$(pl_segment_hasafter $BWhi$On_Yel `date +"%m.%d-%H.%M.%S"` $Yel $On_Gre)\\]""
    if [[ $BRANCH != "" ]]; then # Check if there is a branch in this directory.
        PS1+="\\[$(pl_segment_hasafter $BWhi$On_Gre "${PBranch} ${BRANCH}" $Gre $On_Blu)\\]"
    fi

    PS1+="\\[$(pl_segment_hasafter $BWhi$On_Blu "${USER}@${HOSTNAME}" $Blu $On_Cya)\\]" # Add user@hostname
    PS1+="\\[$(pl_segment $BWhi$On_Cya "${pwd}" $Cya)\\]" # Add current directory
    PS1+="\n" # Add new line.
    PS1+=$'\xe2\x9e\xa5 '
}
...
if [ "$TERM" == "eterm-color" ] || [ "$TERM" == "xterm-256color" ] || [ "$TERM" == "rxvt-unicode" ]; then
